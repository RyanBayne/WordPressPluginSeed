 ...
 * @version     1.0.0
 */
class WPSeed_ListTable_Demo extends WP_List_Table {
	...
	public function __construct() {
		...
		parent::__construct( array(
			'singular'  => __( 'Stock', 'wpseed' ),
			'plural'    => __( 'Stock', 'wpseed' ),
			'ajax'      => false
		) );
    ...
    */
	...
	public function get_columns() {
		...
		$columns = array(
            'headerone'      => __( 'Header One', 'wpseed' ),
			'headertwo'       => __( 'Header Two', 'wpseed' ),
			'headerthree'  => __( 'Header Three', 'wpseed' ),
			'headerfour' => __( 'Header Four', 'wpseed' ),
		);

        // Sub view might offer bulk actions and require checkbox column. 
        if( isset( $this->checkbox_column ) && $this->checkbox_column === true ) {
            $cb = array( 'cb' => __( '<input type="checkbox" />', 'wpseed' ) );
	...
	public function prepare_items() {
		...
		$per_page              = apply_filters( 'wpseed_admin_list_table_demo_items_per_page', 20 );
