<?php         
/**
 * WPSeed - Options Table Interface
 *
 ...
 * any redundent code. 
 * 
 * @todo Once WP Seed is released, consider removing installation related methods.  
 *
 * @author   Ryan Bayne
 ...
 * @version 1.0 
 */
class WPSeed_Options {
    use WPSeed_OptionsTrait;
}

trait WPSeed_OptionsTrait {
    ...
    */
    private static $grouped_options = array(      
        'compact' => 'wpseed_options',
        'private' => 'wpseed_private_options',
    );
    ...
    */
    public static function get_option_names( $type = 'compact' ) {           
        switch ( $type ) {
            ...
            //Individual options here, will be prepended with "wpseed".
            case 'non_compact' :
                return array(
        ...
        */
        ...
        switch ( $type ) {        
            case 'single':
                ...
                // Remember the real option names are prepend with "wpseed".
    ...
    */
    ...
    /**
     * Returns the requested option.  Looks in wpseed_options group 
     * or wpseed_$name as appropriate.
     *
     * @version 1.2
    ...
    public static function get_option( $name, $default = false, $maybe_unserialize = true ) {
        ...
        if ( self::is_valid( $name, 'non_compact' ) ) {
            $option_value = get_option( "wpseed_$name", $default );
    ...
    public function update_option( $name, $value, $autoload = null ) {    HERE
        if ( self::is_valid( $name, 'non_compact' ) ) {
            ...
            if ( version_compare( $GLOBALS['wp_version'], '4.2', '>=' ) ) {
                return update_option( "wpseed_$name", $value, $autoload );
            }
            return update_option( "wpseed_$name", $value );
        }
