<?php
/**
 * WPSeed - Data Formatting Functions
 *
 * @author   Ryan Bayne
 * @category Core
 * @package  WPSeed/Core
...
* @version 1.0
*/
function wpseed_string_half( $string, $ret = null ) {        
    $a = array();
 ...
 */
function wpseed_normalize_postcode( $postcode ) {          
 ...
 * @return string
 */
function wpseed_format_phone_number( $tel ) {            
 ...
 * @return string
 */
function wpseed_strtolower( $string ) {                    
 ...
 * @return string
 */
function wpseed_trim_string( $string, $chars = 200, $suffix = '...' ) {      
 ...
 * @return string|array
 */
function wpseed_clean( $var ) {
    if ( is_array( $var ) ) {
        return array_map( 'wpseed_clean', $var );
    } else {
